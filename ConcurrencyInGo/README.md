...
- [3장 Go의 동시성 구성 요소](#3장-go의-동시성-구성-요소)
  - [고루틴](#고루틴)
  - [sync 패키지](#sync-패키지)
    - [WaitGroup](#waitgroup)
  ...
  - [채널](#채널)
  - [select 구문](#select-구문)
    - [여러 채널에 읽을 내용이 있다면](#여러-채널에-읽을-내용이-있다면)
    - [어떠한 채널도 준비되지 않는다면](#어떠한-채널도-준비되지-않는다면)
    - [무언가 하고 싶지만 현재 준비된 채널이 없다면](#무언가-하고-싶지만-현재-준비된-채널이-없다면)
- [4장 Go의 동시성 패턴](#4장-go의-동시성-패턴)
  - [for-select 루프](#for-select-루프)
...
fmt.Println(<-stringStream) // 비어 있는 채널에서 읽으려는 main 고루틴은 적어도 하나의 항목이 있을때까지 기다림!
```
...
<- 연산자의 수신 형식은 다음과 같이 선택적으로 두 개의 값을 반환할 수도 있다.
```go
stringStream := make(chan string)
go func() {
    stringStream <- "Hello channels!"
...
읽기 연산은 이 두 번째 리턴 값을, 채널의 첫 번째 값이 프로세스 어딘가에서의 쓰기 연산을 통해 생성된 값인지 아니면 닫힌 채널에서 생성되는 기본값인지 나타내기 위해 사용한다.

닫힌 채널은 프로그램에서 더 이상 값이 채널을 통해 전송되지 않는다는 것을 나타낼 수 있기 때문에 매우 유용하다. 그리고 닫힌 채널에서도 값을 읽을 수 있다. 이는 해당 채널에 데이터를 쓰는 상류의 고루틴이 하나뿐이더라도 여러 개의 하류 고루틴이 데이터를 읽을 수 있도록 지원하기 위한 것이다.

채널을 닫는 것은 여러 개의 고루틴에 동시에 신호를 보낼 수 있는 방법 중 하나이기도 하다. 하나의 채널을 기다리고 있는 n개의 고루틴이 있다면, 각 고루틴의 대기를 해제하기 위해 n번씩 채널에 쓰는 대신 간단히 채널을 닫아버려도 된다. 다음은 여러 개의 고루틴으로 한 번에 대기 해제하는 예제이다.
...
대기(blocking)에 대해 이야기하면서 채널에 쓰려고 할 때 채널이 가득 찼다면 빈 공간이 생길 때까지 대기해야 하고, 읽으려고 할 때 채널이 비어 있으면 데이터가 써질 때까지 대기해야 한다고 했던 것을 기억하는가? "가득 차"거나 "비어 있음"은 용량 또는 버퍼 크기에 대한 함수이다. 버퍼링되지 않은 채널의 용량은 0이기 때문에 쓰기 전에 이미 가득 차 있다. 수신자가 없고 버퍼 용량이 4인 버퍼링 된 채널은 4회 쓰기 후에 가득 차고, 다섯 번째 쓰기가 발생하면 다섯 번째 항목을 둘 곳이 없기 때문에 대기시킨다. 이처럼 버퍼링된 채널은 동시에 실행되는 프로세스가 통신할 수 있는 메모리 내의 FIFO 대기열이다.

채널을 올바른 상황에 배치하기 위해 가장 먼저 해야 할 일은 채널의 소유권을 할당하는 것이다. 단방향 채널 선언은 채널을 소유한 고루틴과 단지 채널을 사용하기만 하는 고루틴을 구분할 수 있는 도구이다. 채널 소유자는 채널에 대한 쓰기 접근 권한 측면(chan 또는 chan<-)을 가지고 있으며, 채널의 활용자는 읽기 전용 측면(<-chan)을 가지고 있다.
...
채널의 소비자로서 두 가지 사항만 신경 쓰면 된다.

1. 언제 채널이 닫히는지 아는 것
2. 어떤 이유로든 대기가 발생하면 책임있게 처리하는 것

```go
chanOwner := func() <-chan int {
    resultStream := make(chan int, 5) // 버퍼링된 채널을 인스턴스화
    go func() { // 익명의 고루틴 시작
        defer close(resultStream) // resultStream이 끝나면 닫히도록 보장
        for i := 0; i <= 5; i++ {
            resultStream <- i // resultStream 채널에 쓰기 수행
        }
    }()
    return resultStream // 읽기 전용 채널로 반환
}
...
## select 구문
select문은 채널을 하나로 묶는 접착제이다. switch 블록과 달리 select 블록의 case문은 순차적으로 테스트되지 않으며, 조건이 하나도 충족되지 않는다고 다음 조건으로 넘어가지도 않는다. 대신 select 블록은 채널 중 하나가 준비(읽기의 경우 채워지거나 닫힌 채널, 쓰기의 경우 쓸 수있는 채널)됐는지 확인하기 위해 모든 채널 읽기와 쓰기를 동시에 고려한다. 준비된 채널이 없는 경우 select문 전체가 중단돼 대기한다. 그런 다음 채널들 중 하나가 준비되면 해당 연산이 진행되고 관련 구문들이 실행된다.

무언가 일어나기를 기다리는 동안 대기하는 간단하고 효율적인 방법이다. 그러나 잠시 되돌아보면 몇 가지 의문이 생길 수 있다.
